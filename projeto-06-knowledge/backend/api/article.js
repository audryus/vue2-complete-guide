...
module.exports = app => {
    ...
    const save = (req, res) => {
        ...
        try {
            ...
            existsOrError(article.categoryId, 'Category nao informado')
            existsOrError(article.userId, 'User nao informado')
    ...
    const remove = async (req, res) => {
        try {
            ...
            try {
                existsOrError(rowsDeleted, 'Article nao encontrado')
                res.status(204).send()
            } catch (error) {
                res.status(400).send(error)
            }
